...
bot:Bot|None=None; polling_task:asyncio.Task|None=None; reminder_task:asyncio.Task|None=None
    ...
    while True:
        ...
        for item in items:
            with suppress(Exception):
                await bot.send_message(item['telegram_id'],f"Напоминаю: завтра в {item['starts_at'][11:16]} у вас {item['service']}, мастер {item['staff']}.")
...
class WebBooking(BaseModel):
    service_id:int; staff_id:int; day:str; time:str
    customer_name:str=Field(min_length=2,max_length=80); phone:str=Field(min_length=10,max_length=30)
...
async def public_bootstrap(): return {"services":await db.services(),"staff":await db.staff(),"today":date.today().isoformat(),"max_day":(date.today()+timedelta(days=30)).isoformat(),"business":{"name":"Тихо","address":"ул. Ленина, 10"}}
...
async def public_slots(staff_id:int,day:str):
    ...
    if selected<date.today() or selected>date.today()+timedelta(days=30): raise HTTPException(400,"Дата вне диапазона записи")
...
async def create_public_booking(payload:WebBooking,request:Request):
    try:
        validate_origin(request); phone=normalize_phone(payload.phone); name=validate_name(payload.customer_name)
    ...
    try: booking_id,risk=await db.secure_web_booking(ip_hash=hash_ip(client_ip(request)),phone=phone,name=name,service_id=payload.service_id,staff_id=payload.staff_id,starts_at=starts_at,token=token)
...
async def dashboard():
    today=datetime.now().date().isoformat(); bookings=await db.rows("SELECT b.id,b.customer_name,b.phone,b.starts_at,b.status,b.risk_score,s.name service,s.price,st.name staff FROM bookings b JOIN services s ON s.id=b.service_id JOIN staff st ON st.id=b.staff_id WHERE b.starts_at LIKE ? ORDER BY b.starts_at",(f"{today}%",))
...
@app.get("/health")
async def health(): return {"ok":True,"bot":bool(settings.bot_token),"demo_mode":settings.demo_mode,"pwa":True,"anti_fraud":True}
