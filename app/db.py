...
async def init_db():
    ...
    count = (await (await db.execute("SELECT COUNT(*) n FROM services")).fetchone())["n"]
    if not count:
        await db.executemany("INSERT INTO services(name,duration,price) VALUES(?,?,?)", [("Женская стрижка",60,2000),("Окрашивание",120,5500),("Укладка",45,1800),("Консультация колориста",30,0)])
        await db.executemany("INSERT INTO staff(name) VALUES(?)", [("Анна",),("Елена",)])
        await db.executemany("INSERT INTO settings(key,value) VALUES(?,?)", [("hold_minutes","5"),("minimum_notice_hours","2"),("reminder_hours","24")])
    await db.commit(); await db.close()

async def rows(sql, params=()):
    db=await connect(); cur=await db.execute(sql,params); result=[dict(x) for x in await cur.fetchall()]; await db.close(); return result
...
async def free_slots(staff_id:int, day:str):
    taken={x['starts_at'][11:16] for x in await rows("SELECT starts_at FROM bookings WHERE staff_id=? AND starts_at LIKE ? AND status='confirmed'",(staff_id,f"{day}%"))}
    return [t for t in ["10:00","11:30","13:00","15:00","17:30","19:00"] if t not in taken and datetime.fromisoformat(f"{day}T{t}") > datetime.now()]
